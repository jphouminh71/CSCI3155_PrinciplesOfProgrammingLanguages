...
class CanvasTests extends FunSuite {
    ...
    test("Test Bounding Box with Polygon ") {
        ...
        val poly = new Polygon(pointsList)  // creates a new polgyon
        println(poly.getBoundingBox)
        assert(poly.getBoundingBox == (-1.0, 3.0,-3.5,5.0))  // checks to see if you got the coordinates correctly returned for the shape
        val myc = new MyCanvas(List(poly))  // creates a new canvas that has contains list of polygons, in this case just one they gave you
        println(myc.getBoundingBox)   // so you essentially need to iterate through and find the bounding box of all shapes.
        assert(myc.getBoundingBox == (-1.0, 3.0, -3.5, 5.0))
    }

    test("Test Bounding Box with Circle ") {

        val cir = new MyCircle((-2.0, 3.0), 1.0)        // creating a new circle
        println(cir.getBoundingBox)                     // getting the bounding via member function
        assert(cir.getBoundingBox == (-3.0, -1.0,2.0,4.0))  // seeing if it was correct from the member function
        val myc = new MyCanvas(List(cir))       // creating a canvas with that circle inside of it
        println(myc.getBoundingBox)             // using the canvas function to get the boundingBox for circle inside of it
    ...
    test("testing just this weird polygon"){
        val poly1 = new Polygon(List((0,0.5), (-0.5,1), (-0.25, 2.1)))
        assert(poly1.getBoundingBox == (-0.5, 0, 0.5, 2.1))
        println("passed test3")
    }
